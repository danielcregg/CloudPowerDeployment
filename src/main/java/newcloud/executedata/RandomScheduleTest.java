...
/**
 * Test harness for the random VM placement strategy (baseline).
 */
public class RandomScheduleTest {
    ...
    public List<Double> execute() throws Exception {
        for (int i = 0; i < Iteration; i++) {
            ...
            VmAllocationAssignerRandom assigner = new VmAllocationAssignerRandom(GenExcel.getInstance());

            DatacenterCharacteristics chars = new DatacenterCharacteristics(
                    "x86", "Linux", "Xen", hostList, 10.0, 3.0, 0.05, 0.001, 0.0);
            PowerDatacenterRandom datacenter = new PowerDatacenterRandom(
